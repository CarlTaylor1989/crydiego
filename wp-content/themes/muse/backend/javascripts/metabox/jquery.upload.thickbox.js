...
jQuery(document).ready(function($) {
	...
	$('.gp-upload-thickbox').click( function() {
		
		var $this = $(this),
			$holder = $this.siblings('.gp-images'),
			post_id = $('#post_ID').val(),
		...
		window.send_to_editor = function(html){
			...
			html = '<li id="item_' + id + '">';
			html += '<img src="' + url + '" />';
			html += '<div class="gp-image-bar">';
			html += '<a class="gp-delete-file" href="#" data-field_id="' + field_id + '" data-attachment_id="' + id + '">Delete</a>';
			html += '</div>';
