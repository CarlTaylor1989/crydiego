...
function iron_import_default_data() {
	...
	if ( @file_exists($file) )
	{
		...
		if ( @copy($file, $importPath . 'tmp.xml') )
		{
...
function iron_import_assign_templates() {
	...
	if($errors == 0)
		...
		$data['msg'] .= '<p style="color: red;"><strong>Failed To Assign Static Pages!</strong></p>';
...
function iron_widgets_init() {
	...
	if ( function_exists('get_iron_option') ) :
		...
		$params = array(
			...
			, 'after_widget'  => '</aside>'
			, 'before_title'  => '<div class="panel__heading">%1$s<h3 class="widget-title">'
			, 'after_title'   => '</h3></div>'
		);
	...
	register_widget('Iron_Widget_Radio');
	register_widget('Iron_Widget_Twitter');
	register_widget('Iron_Widget_Terms');
	register_widget('Iron_Widget_Recent_Posts');
	register_widget('Iron_Widget_Recent_Videos');
	register_widget('Iron_Widget_Upcoming_Events');
	register_widget('Iron_Widget_Newsletter');
}
...
function iron_adjust_widget_areas ($params) {
	...
	if ( ! in_array($params[0]['widget_name'], $iron_widgets) )
	{
