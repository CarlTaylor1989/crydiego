...
function iron_get_playlist() {
	...
	if ( empty($post) || $post->ID !== get_option('page_on_front') ) {
...
function iron_the_gig_date ( $d = '' ) {
	...
	echo '<time class="datetime" datetime="' . apply_filters( 'the_gig_timestamp', ( $show_time ? get_the_time('c') : get_the_time('Y-m-d\TZ') ), $d ) . '">' . apply_filters( 'the_gig_date', iron_get_the_gig_date( $d ), $d ) . '</time>';
}
...
function iron_widget_html_title ( $title = '', $instance = array(), $id_base = 0 )
{
...
add_filter('widget_title', 'iron_widget_html_title', 10, 3);



function iron_widget_title ( $post_type = 'post', $page_id = 0 ) {

	$post_type_object = get_post_type_object( $post_type );

	$n = ( empty($post_type_object) ? '' : 's' );

	if ( get_iron_option( $post_type . $n . '_widget_label' ) )
		$title = get_iron_option( $post_type . $n . '_widget_label' );

	else if ( $page_id )
		$title = get_the_title($page_id);

	else if ( get_iron_option( $post_type . $n . '_type_label' ) )
		$title = get_iron_option( $post_type . $n . '_type_label' );
...
function iron_widget_action_link ( $post_type = 'post', $page_id = 0, $url = false ) {
	...
	if ( $page_id || $url )
	{
		...
		if ( ! $action_label = get_iron_option( $post_type . $n . '_widget_action_label' ) ) {
		...
		if ( ! $icon = get_iron_option( $post_type . $n . '_widget_action_icon' ) )
			$icon = 'plus';

		if ( ! $url )
			$url = get_permalink($page_id);

		return '<a class="link panel-action" href="' . $url . '"><i class="panel-action__icon icon-' . $icon . '"></i> <span class="panel-action__label">' . $action_label . '</span></a>';
	}
